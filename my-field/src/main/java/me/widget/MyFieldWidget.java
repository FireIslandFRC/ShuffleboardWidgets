...
@Description(name = "MyField", dataTypes = FieldData.class)
@ParametrizedController("MyFieldWidget.fxml")
public class MyFieldWidget extends SimpleAnnotatedWidget<FieldData> {
  @FXML
  private Pane root;
  ...
  private void initialize() {
    ...
    robot.setImage(
            new Image(getClass().getResource("my-field/robot.png").toExternalForm()));
  ...
  private void updateObjects(boolean forceUpdateObjects) {
    ...
    for (Map.Entry<String, FieldData.SimplePose2d[]> entry : dataOrDefault.get().getObjects().entrySet()) {
      ...
      if (!colors.containsKey(key)) {
        colors.put(key, Color.valueOf(Integer.toHexString(new Random().nextInt(0xFFFFFF) + 0x1000000).substring(1)));
      }
